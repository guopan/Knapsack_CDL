...
void paraDialog::initial_para()
{
    ...
    connect(ui->radioButton_continousdete,&QRadioButton::clicked,this,&paraDialog::set_continusdetekey);        //连续探测
...
void paraDialog::show_detect_mode()
{
    if(psetting.elevationAngle == 0)
    {
        if(psetting.step_azAngle == 0)
            ui->lineEdit_detectDir->setText(QString::fromStdString("水平单向探测"));        //Horizontal single
        else
            ui->lineEdit_detectDir->setText(QString::fromStdString("水平扫描探测，每周方向数")+QString::number(360/psetting.step_azAngle));//Horizontal scanning，directions
    }
    else
        if(psetting.step_azAngle == 0)
            ui->lineEdit_detectDir->setText(QString::fromStdString("单向径向探测"));//Single radial
        else
            ui->lineEdit_detectDir->setText(QString::fromStdString("圆锥扫描探测，每周方向数")+QString::number(360/psetting.step_azAngle));
}
...
//        return;
//    }
    profile_path = QFileDialog::getSaveFileName(this,QString::fromStdString("保存"),".","*.ini");
